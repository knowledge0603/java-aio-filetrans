...
public class AioTcpServer implements Runnable { 
    ...
    public AioTcpServer(int port) throws Exception { 
        //创建线程池
        ExecutorService executor = Executors.newFixedThreadPool(20); //20个线程?
        //异步通道管理器
        asyncChannelGroup = AsynchronousChannelGroup.withThreadPool(executor);  //管理器允许最多20个线程
        //创建 用在服务端的异步Socket.以下简称服务器socket。
        //异步通道管理器，会把服务端所用到的相关参数
        listener = AsynchronousServerSocketChannel.open(asyncChannelGroup).
                bind(new InetSocketAddress(port)); //绑定
    } 
...
//            AioAcceptHandler acceptHandler = new AioAcceptHandler();
            ...
            listener.accept(listener, new AioAcceptHandler()); //开始监听
    ...
    public static void main(String... args) throws Exception { 
        AioTcpServer server = new AioTcpServer(1009); //设置端口
        new Thread(server).start(); //启动线程
    } 
}
